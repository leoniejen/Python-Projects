...
from campy.gui.events.timer import pause  # 自己加上 timer 需要 pause?
import random
...
is_timer_on = False  # control whether the timer is on or not
...
class BreakoutGraphics:
    def __init__(self, ball_radius=BALL_RADIUS, paddle_width=PADDLE_WIDTH, paddle_height=PADDLE_HEIGHT,
        ...
        # start btn - start_btn_word
        self.start_btn_word = GLabel('START', x=(self._window_width - self.start_btn_bg_width)/2+17, y=self._ball.y+43)
        self.start_btn_word.font = 'Helvetica-20-bold'
        self.start_btn_word.color = 'white'

        # score board
        self._score_label = GLabel(f'Score: {self.score}')
        self._score_label.font = 'Helvetica-20-bold'
        self._score_label.color = 'white'
        self._window.add(self._score_label, 5, self._score_label.height + 5)

        # lives board
        self._lives_label = GLabel(f'Lives: {self.lives}')
        self._lives_label.font = 'Helvetica-20-bold'
        self._lives_label.color = 'white'
        self._window.add(self._lives_label, self._window_width - self._lives_label.width - 5,
                         self._lives_label.height + 5)

        # timer
        self._countdown_label = GLabel('3')
        self._countdown_label.font = 'Helvetica-80-bold'
        self._countdown_label.color = 'red'
    ...
    def welcome_screen(self):
        ...
        shows start_btn
        """
        self._window.add(self.start_btn_bg)
        self._window.add(self.start_btn_word)
    ...
    def start_game_click(self, mouse):
        ...
        if is_waiting_click:
            ...
            # if mouse_get is not None: # 問助教：要加這句嗎
            if mouse_get == self.start_btn_word or mouse_get == self.start_btn_bg:
                is_waiting_click = False
                self._window.remove(self.start_btn_word)
                self._window.remove(self.start_btn_bg)
                is_timer_on = True
                self.timer()

    def timer(self):
        """
        when click start_btn, show timer
        countdown form 3 to 1
        """
        global is_timer_on
        global is_in_game
        if is_timer_on:
            for i in range(2, -1, -1):
                self._countdown_label.text = str(i + 1)
                self._window.add(self._countdown_label, x=(self._window_width - self._countdown_label.width) / 2,
                                 y=(self._window_height + self._countdown_label.height) / 2)
                pause(1000)
            self._window.remove(self._countdown_label)
            is_timer_on = False
            is_in_game = True
            self.init_when_start_game()
    ...
    def init_when_start_game(self):
        """
        things need to do when game starts
        """
        if is_in_game:
    ...
    def game_win(self):
        ...
        self._window.clear()
        self._window.add(self.bg_color)
        game_win_label = GLabel(f'Congrats!\n\nYou\'ve won!')
        game_win_label.font = 'Helvetica-36-bold'
        game_win_label.color = 'Gold'
        self._window.add(game_win_label, 20, (self._window_height + game_win_label.height) / 2 - 20)
